...
func CreatedPost(post structTypes.PostInfo) error {
	...
	query := `INSERT INTO post_table (type_name, user_name, title, introduction, content, created_date, updated_date, is_public) 
			  VALUES (?, ?, ?, ?, ?, ?, ?)`

	_, err := DB.Exec(query, post.TypeName, post.UserName, post.Title, post.Introduction, post.Content, time.Now(), time.Now(), post.IsPublic)
	if err != nil {
...
func UpdatePostByID(postID int, post structTypes.Post) error {
	query := `UPDATE post_table SET type_name = ?, user_name = ?, title = ?, introduction=?, content = ?, updated_date = ?, is_public = ? WHERE post_id = ?`
	_, err := DB.Exec(query, post.TypeName, post.UserName, post.Title, post.Introduction, post.Content, time.Now(), post.IsPublic, postID)
	if err != nil {
